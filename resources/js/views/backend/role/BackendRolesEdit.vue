...
export default {
    ...
    async created() {
        this.isLoading = true;
        const url = "admin/roles/" + this.$route.params.id + "/edit";
    ...
    methods: {
        async updateRole() {
            ...
            const url = "admin/roles/" + this.$route.params.id;
            const res = await this.callApi("put", url, this.form);
            if (res.status === 200) {
                this.$router.push({ name: "BackendRolesIndex" });
