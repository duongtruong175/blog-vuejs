        ...
        <div v-if="!isLoading" class="w-full sm:max-w-4xl mt-6 px-6 py-4 bg-white sm:rounded-lg">
            <form @submit.prevent="updateTag">
                ...
                <div class="mt-8 flex-row">
                    ...
                    <router-link :to="{ name: 'BackendTagsIndex' }" class="ml-3 inline-flex items-center px-4 py-2 bg-blue-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-900 focus:outline-none focus:border-blue-900 focus:ring ring-blue-300 disabled:opacity-25 transition ease-in-out duration-150">
                        {{ $t('Cancel') }}
                    </router-link>
                </div>
            </form>
...
export default {
    ...
    async created() {
        this.isLoading = true;
        const url = "admin/tags/" + this.$route.params.id + "/edit";
    ...
    methods: {
        async updateTag() {
            ...
            const url = "admin/tags/" + this.$route.params.id;
            const res = await this.callApi("put", url, this.form);
            if (res.status === 200) {
                this.$router.push({ name: "BackendTagsIndex" });
